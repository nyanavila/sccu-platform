...
function MemberPortal({ onBack }) {
  ...
  const [lnF,setLnF]         = useState({type:CFG.LOANS[0]?.name||"",amount:"",purpose:""});
  const [lnStep,setLnStep]   = useState("form"); // form | submitting | done
  ...
  return (
    <div style={{minHeight:"100vh",background:C.cream,fontFamily:CFG.FONTS.body}}>
      ...
      <Modal open={loanOpen} onClose={()=>{setLoanOpen(false);setLnStep("form");setLnErr("");}} title="Apply for a loan">
        {lnStep==="done"
          ? <Ok msg={`Application received! A loan officer will contact you at ${member?.email} within 3 business days.`}/>
          : <>
            <Fld label="Loan type">
              <select style={inp} value={lnF.type} onChange={e=>setLnF(f=>({...f,type:e.target.value}))}>
                {CFG.LOANS.map(l=><option key={l.name} value={l.name}>{l.name} — from {l.rate}</option>)}
              </select>
            </Fld>
            <Fld label={`Amount (${CFG.CURRENCY})`}>
              <input style={inp} type="number" value={lnF.amount} onChange={e=>setLnF(f=>({...f,amount:e.target.value}))} placeholder="0.00"/>
            </Fld>
            <Fld label="Purpose">
              <textarea style={{...inp,height:80,resize:"vertical"}} value={lnF.purpose} onChange={e=>setLnF(f=>({...f,purpose:e.target.value}))} placeholder="Describe how you will use the funds…"/>
            </Fld>
            <Err msg={lnErr}/>
            <button
              disabled={lnStep==="submitting"||!lnF.amount}
              onClick={async()=>{
                if(!lnF.amount){setLnErr("Enter an amount");return;}
                setLnStep("submitting");setLnErr("");
                try {
                  const {ok,data}=await gwPost("/loan-applications",{
                    institution:SH,name:member?.name,email:member?.email,
                    account_id:accounts[0]?.id,loan_type:lnF.type,
                    amount:parseFloat(lnF.amount),purpose:lnF.purpose,currency:CFG.CURRENCY,
                  });
                  if(!ok) throw new Error(data.detail||"Submission failed");
                  setLnStep("done");
                } catch(e){setLnErr(e.message);setLnStep("form");}
              }}
              style={{...mkBtn(C.primary),width:"100%",opacity:!lnF.amount?.5:1}}>
              {lnStep==="submitting"?"Submitting…":"Submit application"}
            </button>
          </>
        }
      </Modal>
...
function TellerPortal({ onBack }) {
  const TAPI = `http://127.0.0.1:${CFG.TELLER_API_PORT}`;
  const [step,setStep]         = useState("login");
  ...
  const doLogin = async () => {
    ...
    setLoading(true);setErr("");
    try {
      const r=await fetch(`${TAPI}/teller/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:user,password:pass})});
...
function AdminPortal({ onBack }) {
  ...
  const doLogin = async () => {
    ...
    setLoading(true);setErr("");
    try {
      const {ok,data}=await gwPost("/auth/login",{username:user,password:pass,institution:SH});
      if(!ok) throw new Error(data.detail||"Login failed");
  ...
  const createAccount = async () => {
    if(!nF.name||!nF.email){setSaveMsg("");return;}
    setSaving(true);setSaveMsg("");
    try {
      // Step 1: Create OBP user via gateway → POST /obp/v5.1.0/users
      const {ok:uOk,data:uData}=await gwPost(`/institutions/${SH}/create-user`,{
        email:nF.email, username:nF.email.split("@")[0],
        first_name:nF.name.split(" ")[0], last_name:nF.name.split(" ").slice(1).join(" ")||".",
        membership_type:nF.type,
      });
      if(!uOk) throw new Error(uData.detail||"User creation failed");
      // Step 2: Create account at zero balance (OBP rejects non-zero initial balance)
      const {ok:aOk,data:aData}=await gwPost(`/institutions/${SH}/create-account`,{
        user_id:uData.user_id, label:"Share Savings",
        product_code:"CURRENT", currency:CFG.CURRENCY,
        opening_deposit:parseFloat(nF.deposit)||0,
      });
      if(!aOk) throw new Error(aData.detail||"Account creation failed");
      setSaveMsg(`✓ Account created for ${nF.name} — opening deposit of ${CFG.CURRENCY} ${nF.deposit} must be posted via teller portal`);
      setNF({name:"",email:"",type:CFG.MEMBER_TYPES[0]||"",deposit:"500"});
      loadAll();
      setTimeout(()=>{setNewOpen(false);setSaveMsg("");},4000);
    } catch(e){
...
// ══════════════════════════════════════════════════════════════
//  LOAN PORTAL  (/loans)
//  Standalone public-facing loan application portal.
//  Members and non-members can apply; submission goes to gateway.
// ══════════════════════════════════════════════════════════════
function LoanPortal({ onBack }) {
  const [lF, setLF] = useState({
    name:"", email:"", phone:"", type:CFG.LOANS[0]?.name||"",
    amount:"", purpose:"", employment:"", income:"",
  });
  const [step, setStep]       = useState("form"); // form | submitting | done | error
  const [errMsg, setErrMsg]   = useState("");

  const submit = async () => {
    if (!lF.name || !lF.email || !lF.amount) {
      setErrMsg("Name, email and amount are required."); return;
    }
    ...
    try {
      const { ok, data } = await gwPost("/loan-applications", {
        ...
        phone:       lF.phone,
        loan_type:   lF.type,
        amount:      parseFloat(lF.amount),
        purpose:     lF.purpose,
        employment:  lF.employment,
        income:      lF.income ? parseFloat(lF.income) : undefined,
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.cream, fontFamily:CFG.FONTS.body }}>
      <link href={CFG.FONTS.google} rel="stylesheet"/>
      {/* Header */}
      <div style={{ background:C.primary, padding:"14px 40px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:36, height:36, borderRadius:8, background:C.accent, display:"flex", alignItems:"center", justifyContent:"center" }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 2C8 2 4 5 4 10c0 6 8 12 8 12s8-6 8-12c0-5-4-8-8-8z" fill="white"/></svg>
          </div>
          <div style={{ fontFamily:CFG.FONTS.display, fontSize:16, fontWeight:700, color:C.white }}>{SH.toUpperCase()} Loan Application</div>
        </div>
        <button onClick={onBack} style={mkBtn("rgba(255,255,255,.15)",C.white,{padding:"6px 16px",fontSize:13,border:"1px solid rgba(255,255,255,.25)"})}>← Back to site</button>
      </div>

      <div style={{ maxWidth:640, margin:"48px auto", padding:"0 24px" }}>
        {step==="done" ? (
          <div style={{ ...crd, textAlign:"center", padding:"60px 40px" }}>
            <div style={{ width:64, height:64, borderRadius:"50%", background:C.success+"18", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 20px", fontSize:28, color:C.success }}>✓</div>
            <div style={{ fontFamily:CFG.FONTS.display, fontSize:26, fontWeight:700, color:C.primary, marginBottom:12 }}>Application received!</div>
            <div style={{ fontSize:15, color:C.muted, lineHeight:1.7, marginBottom:32 }}>
              Thank you, {lF.name}. A loan officer will review your application and contact you at <strong>{lF.email}</strong> within 3 business days.
            </div>
            <div style={{ background:C.cream, borderRadius:10, padding:"16px 20px", marginBottom:32, fontSize:13, color:C.muted, textAlign:"left" }}>
              <div style={{ fontWeight:600, color:C.ink, marginBottom:8 }}>Your application summary</div>
              {[["Loan type",lF.type],["Amount requested",`${CFG.CURRENCY} ${parseFloat(lF.amount).toLocaleString()}`],["Purpose",lF.purpose||"—"],["Contact",lF.email]].map(([k,v])=>(
                <div key={k} style={{ display:"flex", justifyContent:"space-between", padding:"6px 0", borderBottom:`1px solid ${C.border}` }}>
                  <span>{k}</span><span style={{ fontWeight:600, color:C.ink }}>{v}</span>
                </div>
              ))}
            </div>
            <button onClick={onBack} style={{ ...mkBtn(C.primary), padding:"11px 32px" }}>Back to {SH.toUpperCase()}</button>
          </div>
        ) : (
          <div style={crd}>
            <div style={{ fontFamily:CFG.FONTS.display, fontSize:26, fontWeight:700, color:C.primary, marginBottom:4 }}>Apply for a loan</div>
            <div style={{ fontSize:13, color:C.muted, marginBottom:28 }}>{CFG.FULL_NAME} · {CFG.LOCATION}</div>

            {/* Loan type selector */}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:12, marginBottom:24 }}>
              {CFG.LOANS.map(l=>(
                <div key={l.name} onClick={()=>setLF(f=>({...f,type:l.name}))}
                  style={{ border:`2px solid ${lF.type===l.name?C.primary:C.border}`, borderRadius:10, padding:"14px 16px", cursor:"pointer", background:lF.type===l.name?C.primary+"0A":C.white }}>
                  <div style={{ fontSize:13, fontWeight:700, color:lF.type===l.name?C.primary:C.ink, marginBottom:4 }}>{l.name}</div>
                  <div style={{ fontSize:12, color:C.muted }}>from {l.rate} · up to {l.max}</div>
                </div>
              ))}
            </div>

            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              <Fld label="Full name (legal)">
                <input style={inp} value={lF.name} onChange={e=>setLF(f=>({...f,name:e.target.value}))} placeholder="As on national ID"/>
              </Fld>
              <Fld label="Email address">
                <input style={inp} type="email" value={lF.email} onChange={e=>setLF(f=>({...f,email:e.target.value}))} placeholder={`you@example.com`}/>
              </Fld>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              <Fld label="Phone">
                <input style={inp} value={lF.phone} onChange={e=>setLF(f=>({...f,phone:e.target.value}))} placeholder="+501 XXX-XXXX"/>
              </Fld>
              <Fld label={`Amount (${CFG.CURRENCY})`}>
                <input style={inp} type="number" value={lF.amount} onChange={e=>setLF(f=>({...f,amount:e.target.value}))} placeholder="0.00"/>
              </Fld>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              <Fld label="Employment status">
                <select style={inp} value={lF.employment} onChange={e=>setLF(f=>({...f,employment:e.target.value}))}>
                  {["","Employed","Self-employed","Farmer","Business owner","Retired","Other"].map(o=><option key={o} value={o}>{o||"Select…"}</option>)}
                </select>
              </Fld>
              <Fld label={`Monthly income (${CFG.CURRENCY}, optional)`}>
                <input style={inp} type="number" value={lF.income} onChange={e=>setLF(f=>({...f,income:e.target.value}))} placeholder="0.00"/>
              </Fld>
            </div>
            <Fld label="Loan purpose">
              <textarea style={{...inp, height:90, resize:"vertical"}} value={lF.purpose} onChange={e=>setLF(f=>({...f,purpose:e.target.value}))} placeholder="Describe how you will use the funds…"/>
            </Fld>

            <Err msg={errMsg}/>
            <div style={{ display:"flex", gap:12 }}>
              <button onClick={onBack} style={{...mkBtn(C.cream,C.muted,{border:`1px solid ${C.border}`}),flex:1}}>Cancel</button>
              <button onClick={submit} disabled={step==="submitting"||!lF.name||!lF.email||!lF.amount}
                style={{...mkBtn(C.primary),flex:2,opacity:(!lF.name||!lF.email||!lF.amount)?.5:1}}>
                {step==="submitting"?"Submitting…":"Submit application →"}
              </button>
            </div>

            <div style={{ marginTop:20, padding:"14px 16px", background:C.cream, borderRadius:8, fontSize:12, color:C.muted, lineHeight:1.7 }}>
              By submitting you consent to {CFG.FULL_NAME} processing your personal data for loan assessment purposes.
              Approval is subject to creditworthiness review. Minimum member deposit: {CFG.CURRENCY} {CFG.SAVINGS.min_balance.toFixed(2)}.
            </div>
          </div>
        )}
      </div>
...
export default function InstitutionApp() {
  ...
  if (route==="loans")  return <LoanPortal    onBack={()=>nav("public")}/>;
